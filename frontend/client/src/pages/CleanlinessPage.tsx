...
export default function CleanlinessPage() {
  ...
  const answeredQuestions = activeTemplates.reduce((total, template) => {
    ...
    return total + template.items.filter((item) => inspection.results.some((result) => result.template_item_id === item.id && (item.item_type === "text" ? Boolean(result.value_text?.trim()) : result.passed !== null && result.passed !== undefined))).length;
  }, 0);
  ...
  function draftFor(inspection: Inspection, item: TemplateItem, result?: Result): RowDraft {
    ...
    return { passed: item.item_type === "text" ? (result?.value_text?.trim() ? "yes" : "pending") : result?.passed === true || result?.value_boolean === true ? "yes" : result?.passed === false || result?.value_boolean === false ? "no" : "pending", notes: result?.value_text || result?.notes || "", cleanerId: result?.responsible_cleaner_id ? String(result.responsible_cleaner_id) : "" };
  }
