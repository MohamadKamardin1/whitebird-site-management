...
interface AttendanceRecord { id: number; cleaner_id: number; cleaner_name?: string; cleaner?: unknown; site_id: number; shift_name?: string | null; attendance_date: string; status: string; attendance_outcome?: "present" | "half_present" | "absent" | string; check_in_time?: string | null; check_out_time?: string | null; notes?: string; review_status?: string; is_editable?: boolean; return_reason?: string; }
...
export default function AttendancePage() {
  ...
  const markAbsent = (record: AttendanceRecord) => updateDraft(record.id, { status: "absent", checkIn: "", checkOut: "" });
  const markSignIn = (record: AttendanceRecord) => updateDraft(record.id, { status: "present", checkIn: drafts[record.id]?.checkIn || nowTime() });
  const markSignOut = (record: AttendanceRecord) => updateDraft(record.id, { status: "present", checkOut: drafts[record.id]?.checkOut || nowTime() });
