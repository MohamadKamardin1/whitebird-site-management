...
class Site(TimeStampedModel):
    ...
    site_type = models.ForeignKey(
        SiteType, on_delete=models.PROTECT, related_name="sites", null=True, blank=True
    )
    status = models.ForeignKey(
        SiteStatus, on_delete=models.PROTECT, related_name="sites", null=True, blank=True
    )
    description = models.TextField(blank=True, default="")
...
class Department(TimeStampedModel):
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=["site", "name"], name="uniq_department_site_name")
        ]
...
class Asset(TimeStampedModel):
    ...
    category = models.ForeignKey(
        AssetCategory, on_delete=models.PROTECT, related_name="assets", null=True, blank=True
    )
    name = models.CharField(max_length=160)
    serial_number = models.CharField(max_length=100, blank=True, default="", db_index=True)
    quantity = models.PositiveIntegerField(default=1)
    condition = models.CharField(
        max_length=24, choices=Condition.choices, default=Condition.OPERATIONAL
    )
    is_active = models.BooleanField(default=True)
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["site", "serial_number"], name="uniq_asset_site_serial"
            )
        ]
...
class StaffAssignment(TimeStampedModel):
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=["site", "user"], name="uniq_assignment_site_user")
        ]
