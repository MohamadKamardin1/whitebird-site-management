...
def test_update_site_records_changes_and_audit(admin_user, site):
    ...
    audit = AuditLog.objects.filter(
        entity_type="sites.site", entity_id=site.pk
    ).latest("created_at")
...
def test_status_change_creates_notifications(admin_user, site, site_status):
    maintenance = SiteStatus.objects.create(
        name="Maintenance", slug="maintenance", color="#3b82f6"
    )
...
def test_all_writes_create_audit_entries(site, admin_user):
    ...
    assert AuditLog.objects.filter(entity_type="sites.site", entity_id=created.pk).exists()
