...
from apps.sites.models import Asset
from apps.sites.services import SiteDraft, assign_staff, create_site
...
def test_site_list_staff_sees_only_assigned(admin_client, staff_client, site):
    ...
    response = staff_client.get("/api/v1/sites")
    assert response.status_code == 200
...
def test_site_list_filters(admin_client, site, site_status, site_type):
    response = admin_client.get("/api/v1/sites", {"status": "active", "region": "Unguja North"})
    assert response.status_code == 200
...
def test_site_create_requires_privileged_role(staff_client, manager_client, admin_client):
    ...
    assert _post(staff_client, "/api/v1/sites", payload).status_code == 403
    assert _post(manager_client, "/api/v1/sites", payload).status_code == 200
    assert _post(admin_client, "/api/v1/sites", payload).status_code == 200


@pytest.mark.django_db
def test_site_detail_and_update(admin_client, site, site_status):
    detail = admin_client.get(f"/api/v1/sites/{site.pk}")
...
def test_site_detail_404_for_missing(admin_client):
    assert admin_client.get("/api/v1/sites/999999").status_code == 404


@pytest.mark.django_db
def test_site_archive_and_restore(admin_client, site):
    assert admin_client.delete(f"/api/v1/sites/{site.pk}").status_code == 200
    assert admin_client.get(f"/api/v1/sites/{site.pk}").status_code == 404
    restored = admin_client.post(f"/api/v1/sites/{site.pk}/restore")
...
def test_staff_cannot_archive_site(staff_client, site):
    staff = User.objects.get(username="staff")
    assign_staff(site=site, user=staff, actor=site.created_by)
    assert staff_client.delete(f"/api/v1/sites/{site.pk}").status_code == 403


@pytest.mark.django_db
def test_department_crud(admin_client, site):
    created = _post(admin_client, f"/api/v1/sites/{site.pk}/departments", {"name": "Housekeeping"})
    assert created.status_code == 200
    dept_id = created.json()["id"]

    listed = admin_client.get(f"/api/v1/sites/{site.pk}/departments")
    assert [d["name"] for d in listed.json()] == ["Housekeeping"]

    updated = admin_client.patch(
        f"/api/v1/sites/{site.pk}/departments/{dept_id}",
        data={"description": "Rooms and public areas"},
        content_type=JSON,
    )
    assert updated.status_code == 200

    assert admin_client.delete(f"/api/v1/sites/{site.pk}/departments/{dept_id}").status_code == 200
...
def test_asset_crud(admin_client, site, asset_category):
    ...
    updated = admin_client.patch(
        f"/api/v1/sites/{site.pk}/assets/{asset_id}",
    ...
    assert admin_client.delete(f"/api/v1/sites/{site.pk}/assets/{asset_id}").status_code == 200
...
def test_assignment_flow(admin_client, staff_user, site):
    created = _post(
        admin_client,
        f"/api/v1/sites/{site.pk}/assignments",
        {"user_id": staff_user.pk, "role": "staff"},
    )
    assert created.status_code == 200
    assignment_id = created.json()["id"]

    assert admin_client.get(f"/api/v1/sites/{site.pk}/assignments").status_code == 200
    primary = admin_client.post(
        f"/api/v1/sites/{site.pk}/assignments/{assignment_id}/primary"
    )
    assert primary.status_code == 200
    removed = admin_client.delete(
        f"/api/v1/sites/{site.pk}/assignments/{assignment_id}"
    )
...
def test_stats_endpoints(admin_client, site):
    stats = admin_client.get(f"/api/v1/sites/{site.pk}/stats")
    assert stats.status_code == 200
    assert stats.json()["site_id"] == site.pk

    overview = admin_client.get("/api/v1/stats/overview")
...
def test_catalog_endpoints(admin_client, site_type, site_status, asset_category):
    assert admin_client.get("/api/v1/catalog/types").status_code == 200
    assert admin_client.get("/api/v1/catalog/statuses").status_code == 200
    assert admin_client.get("/api/v1/catalog/categories").status_code == 200


@pytest.mark.django_db
def test_staff_cannot_view_overview_stats(staff_client):
    assert staff_client.get("/api/v1/stats/overview").status_code == 403
...
def test_notifications_flow(admin_client, site, staff_user, admin_user):
    ...
    count = admin_client.get("/api/v1/notifications/unread-count")
    assert count.status_code == 200

    listed = admin_client.get("/api/v1/notifications")
    assert listed.status_code == 200
    if listed.json():
        ids = [n["id"] for n in listed.json()]
        marked = _post(admin_client, "/api/v1/notifications/mark-read", ids)
        assert marked.status_code == 200


@pytest.mark.django_db
def test_audit_logs_require_privilege(staff_client, admin_client):
    assert staff_client.get("/api/v1/audit-logs").status_code == 403
...
def test_openapi_schema_available(anon_client):
    assert anon_client.get("/api/v1/openapi.json").status_code == 200
