...
def _on_status_changed(*, site: Site, actor: User, previous_status_slug: str | None) -> None:
    record_audit(
        ...
        entity=site,
        summary=(
            f"Status changed: {previous_status_slug or '—'} -> "
            f"{site.status.slug if site.status else '—'}"
        ),
    )
    if site.status:
...
def create_department(*, site: Site, name: str, description: str = "", actor: User) -> Department:
    with transaction.atomic():
        if Department.objects.filter(site=site, name__iexact=name, is_active=True).exists():
            raise IntegrityError("A department with this name already exists on the site.")
        department = Department.objects.create(
            site=site, name=name, description=description, created_by=actor
        )
        record_audit(
            action=AuditLog.Action.CREATE,
            actor=actor,
    ...
    with transaction.atomic():
        if name is not None and name.lower() != department.name.lower():
            duplicate = Department.objects.filter(
                site=department.site, name__iexact=name, is_active=True
...
def mark_notifications_read(*, user: User, notification_ids: list[int]) -> int:
    return Notification.objects.filter(
        recipient=user, pk__in=notification_ids, is_read=False
    ).update(is_read=True)
...
def scoped_site_ids(user: User) -> list[int]:
    ...
    return list(
        user.staff_assignments.filter(site__is_active=True).values_list("site_id", flat=True)
    )
